...
class ListSelectionIndicesModel(v: Box[Set[Int]], setFilter: => Boolean, table: JTable) extends ListSelectionModel {
  ...
  val observer = Observer { r=> 
    val newIndices = v(r)
    SwingView.replaceUpdate (this, handleBoxChange(newIndices))
  }
  ...
  private def handleDelegateChange() {
    ...
    if (setFilter) {
      ...
      //a new instance.
      SwingView.addUpdate(this, atomic{
        v() = selection
      })
  ...
  //TODO update this
  private def handleBoxChange(indices:Set[Int]) {
      ...
      if (!indices.sameElements(selection)) {
        ...
        indices.foreach(i => {
          try {
            ...
            case _: IndexOutOfBoundsException => println("Invalid model row index " + i)
          }
        })
